# 1 "0_Src/0_AppSw/Fat_fs/ff.c"
# 1 "C:\\betriebssys\\BaseFramework_TC26xB//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "0_Src/0_AppSw/Fat_fs/ff.c"
# 98 "0_Src/0_AppSw/Fat_fs/ff.c"
# 1 "0_Src/0_AppSw/Fat_fs/ff.h" 1
# 25 "0_Src/0_AppSw/Fat_fs/ff.h"
# 1 "0_Src/0_AppSw/Fat_fs/integer.h" 1
# 16 "0_Src/0_AppSw/Fat_fs/integer.h"
typedef int INT;
...
# 26 "0_Src/0_AppSw/Fat_fs/ff.h" 2
# 1 "0_Src/0_AppSw/Fat_fs/ffconf.h" 1
# 27 "0_Src/0_AppSw/Fat_fs/ff.h" 2
# 68 "0_Src/0_AppSw/Fat_fs/ff.h"
    typedef char TCHAR;
# 81 "0_Src/0_AppSw/Fat_fs/ff.h"
    typedef struct
    {
        BYTE fs_type;
        BYTE drv;
...
# 99 "0_Src/0_AppSw/Fat_fs/ff.c" 2
# 1 "0_Src/0_AppSw/Fat_fs/diskio.h" 1
# 20 "0_Src/0_AppSw/Fat_fs/diskio.h"
...
# 100 "0_Src/0_AppSw/Fat_fs/ff.c" 2
# 469 "0_Src/0_AppSw/Fat_fs/ff.c"
...
static WORD Fsid;
# 490 "0_Src/0_AppSw/Fat_fs/ff.c"
static WCHAR LfnBuf[255 + 1];
# 526 "0_Src/0_AppSw/Fat_fs/ff.c"
static void mem_cpy (void *dst, const void *src, UINT cnt)
{
    BYTE *d = (BYTE *) dst;
    const BYTE *s = (const BYTE *) src;
# 540 "0_Src/0_AppSw/Fat_fs/ff.c"
    while (cnt--)
        *d++ = *s++;
}
    ...
    )
{
    ...
    case 1:
        bc = (UINT) clst;
        bc += bc / 2;
        if (move_window (fs, fs->fatbase + (bc / 512U)))
            break;
        wc = fs->win[bc % 512U];
        bc++;
        if (move_window (fs, fs->fatbase + (bc / 512U)))
    ...
    case 2:
        if (move_window (fs, fs->fatbase + (clst / (512U / 2))))
            break;
        p = &fs->win[clst * 2 % 512U];
        return (WORD)(((WORD)*((BYTE*)(p)+1)<<8)|(WORD)*(BYTE*)(p));

    case 3:
        if (move_window (fs, fs->fatbase + (clst / (512U / 4))))
            break;
...
# 1002 "0_Src/0_AppSw/Fat_fs/ff.c"
            clst = nxt;
        }
    }
...
# 1017 "0_Src/0_AppSw/Fat_fs/ff.c"
...
# 1121 "0_Src/0_AppSw/Fat_fs/ff.c"
...
# 1171 "0_Src/0_AppSw/Fat_fs/ff.c"
...
# 1252 "0_Src/0_AppSw/Fat_fs/ff.c"
...
# 1282 "0_Src/0_AppSw/Fat_fs/ff.c"
...
# 1404 "0_Src/0_AppSw/Fat_fs/ff.c"
void gen_numname (BYTE * dst,
                  const BYTE * src,
                  const WCHAR * lfn,
                  WORD seq
    )
...
# 1461 "0_Src/0_AppSw/Fat_fs/ff.c"
...
# 1481 "0_Src/0_AppSw/Fat_fs/ff.c"
    ...
    do
    {
        ...
        else
        {
            ...
            else
            {
                ...
                ord = 0xFF;
                dj->lfn_idx = 0xFFFF;
                if (!(dj->fn[11] & 0x01) && !mem_cmp (dir, dj->fn, 11))
                    break;
            }
        }
...
# 1560 "0_Src/0_AppSw/Fat_fs/ff.c"
...
# 1631 "0_Src/0_AppSw/Fat_fs/ff.c"
...
# 1743 "0_Src/0_AppSw/Fat_fs/ff.c"
    if (res == FR_OK)
    {
        res = move_window (dj->fs, dj->sect);
        if (res == FR_OK)
...
# 1769 "0_Src/0_AppSw/Fat_fs/ff.c"
...
# 1810 "0_Src/0_AppSw/Fat_fs/ff.c"
    return res;
}
# 1821 "0_Src/0_AppSw/Fat_fs/ff.c"
    ...
    )
{
    ...
    *path = &p[si];
    cf = (w < ' ') ? 0x04 : 0;
# 1876 "0_Src/0_AppSw/Fat_fs/ff.c"
    while (di)
    {
        w = lfn[di - 1];
        if (w != ' ' && w != '.')
    ...
    return FR_OK;
# 2094 "0_Src/0_AppSw/Fat_fs/ff.c"
}
# 2103 "0_Src/0_AppSw/Fat_fs/ff.c"
...
# 2206 "0_Src/0_AppSw/Fat_fs/ff.c"
    ...
    )
{
    FRESULT res;
    BYTE *dir, ns;
# 2226 "0_Src/0_AppSw/Fat_fs/ff.c"
    if (*path == '/' || *path == '\\')
        path++;
    dj->sclust = 0;
...
# 2287 "0_Src/0_AppSw/Fat_fs/ff.c"
...
# 2312 "0_Src/0_AppSw/Fat_fs/ff.c"
    ...
    )
{
    ...
    *rfs = fs;
    ...
    if (!fasize)
        fasize = (DWORD)(((DWORD)*((BYTE*)(fs->win + 36)+3)<<24)|((DWORD)*((BYTE*)(fs->win + 36)+2)<<16)|((WORD)*((BYTE*)(fs->win + 36)+1)<<8)|*(BYTE*)(fs->win + 36));
    fs->fsize = fasize;
    ...
    if (fs->n_rootdir % (512U / 32))
        return FR_NO_FILESYSTEM;

    tsect = (WORD)(((WORD)*((BYTE*)(fs->win + 19)+1)<<8)|(WORD)*(BYTE*)(fs->win + 19));
    if (!tsect)
        tsect = (DWORD)(((DWORD)*((BYTE*)(fs->win + 32)+3)<<24)|((DWORD)*((BYTE*)(fs->win + 32)+2)<<16)|((WORD)*((BYTE*)(fs->win + 32)+1)<<8)|*(BYTE*)(fs->win + 32));
    ...
    if (fmt == 3)
    {
        ...
        if (disk_read (fs->drv, fs->win, fs->fsi_sector, 1) == RES_OK &&
            ...
            (DWORD)(((DWORD)*((BYTE*)(fs->win + 0)+3)<<24)|((DWORD)*((BYTE*)(fs->win + 0)+2)<<16)|((WORD)*((BYTE*)(fs->win + 0)+1)<<8)|*(BYTE*)(fs->win + 0)) == 0x41615252 && (DWORD)(((DWORD)*((BYTE*)(fs->win + 484)+3)<<24)|((DWORD)*((BYTE*)(fs->win + 484)+2)<<16)|((WORD)*((BYTE*)(fs->win + 484)+1)<<8)|*(BYTE*)(fs->win + 484)) == 0x61417272)
        {
...
# 2507 "0_Src/0_AppSw/Fat_fs/ff.c"
...
# 2541 "0_Src/0_AppSw/Fat_fs/ff.c"
...
# 2584 "0_Src/0_AppSw/Fat_fs/ff.c"
...
# 2713 "0_Src/0_AppSw/Fat_fs/ff.c"
        ;
...
# 2740 "0_Src/0_AppSw/Fat_fs/ff.c"
         ...
         rbuff += rcnt, fp->fptr += rcnt, *br += rcnt, btr -= rcnt)
    {
        if ((fp->fptr % 512U) == 0)
        {
            ...
            if (!csect)
            {
                ...
                if (clst < 2)
                    { fp->flag |= 0x80; return FR_INT_ERR; };
                if (clst == 0xFFFFFFFF)
...
# 2855 "0_Src/0_AppSw/Fat_fs/ff.c"
...
# 2957 "0_Src/0_AppSw/Fat_fs/ff.c"
            if (fp->dsect != sect)
            {
...
# 2993 "0_Src/0_AppSw/Fat_fs/ff.c"
...
# 3044 "0_Src/0_AppSw/Fat_fs/ff.c"
FRESULT f_close (FIL * fp
    )
{
    FRESULT res;
# 3061 "0_Src/0_AppSw/Fat_fs/ff.c"
    res = f_sync (fp);
# 3078 "0_Src/0_AppSw/Fat_fs/ff.c"
    if (res == FR_OK)
        fp->fs = 0;
    return res;

}
# 3239 "0_Src/0_AppSw/Fat_fs/ff.c"
    ...
    )
{
    ...
    if (fp->flag & 0x80)
        return FR_INT_ERR;
# 3332 "0_Src/0_AppSw/Fat_fs/ff.c"
    {
...
# 3439 "0_Src/0_AppSw/Fat_fs/ff.c"
...
# 3497 "0_Src/0_AppSw/Fat_fs/ff.c"
...
# 3545 "0_Src/0_AppSw/Fat_fs/ff.c"
...
# 3579 "0_Src/0_AppSw/Fat_fs/ff.c"
        ...
        else
        {
            ...
            else
            {
                ...
                do
                {
                    ...
                    if (fat == 2)
                    {
                        if ((WORD)(((WORD)*((BYTE*)(p)+1)<<8)|(WORD)*(BYTE*)(p)) == 0)
                            n++;
                        p += 2;
                        i -= 2;
                    }
                    ...
                    {
                        if (((DWORD)(((DWORD)*((BYTE*)(p)+3)<<24)|((DWORD)*((BYTE*)(p)+2)<<16)|((WORD)*((BYTE*)(p)+1)<<8)|*(BYTE*)(p)) & 0x0FFFFFFF) == 0)
                            n++;
                        p += 4;
                        i -= 4;
                    }
...
# 3676 "0_Src/0_AppSw/Fat_fs/ff.c"
...
# 3740 "0_Src/0_AppSw/Fat_fs/ff.c"
...
# 3823 "0_Src/0_AppSw/Fat_fs/ff.c"
...
# 3909 "0_Src/0_AppSw/Fat_fs/ff.c"
...
# 3955 "0_Src/0_AppSw/Fat_fs/ff.c"
...
# 4000 "0_Src/0_AppSw/Fat_fs/ff.c"
...
# 4169 "0_Src/0_AppSw/Fat_fs/ff.c"
    ...
    )
{
    ...
    if (disk_ioctl (pdrv, 3, &n) != RES_OK || !n || n > 32768)
        n = 1;
    n = (b_data + n - 1) & ~(n - 1);
    n = (n - b_data) / 1;
    if (fmt == 3)
    ...
    if ((fmt == 2 && n_clst < 4086) || (fmt == 3 && n_clst < 65526))
        return FR_MKFS_ABORTED;
    ...
    *(BYTE*)(tbl + 510)=(BYTE)(0xAA55); *((BYTE*)(tbl + 510)+1)=(BYTE)((WORD)(0xAA55)>>8);
    ...
    while (--i);
# 4436 "0_Src/0_AppSw/Fat_fs/ff.c"
    if (fmt == 3)
    {
...
# 4539 "0_Src/0_AppSw/Fat_fs/ff.c"
    ...
    )
{
    ...
    while (n < len - 1)
    {
        ...
        if (rc != 1)
            break;
        c = s[0];
# 4589 "0_Src/0_AppSw/Fat_fs/ff.c"
        if (c == '\r')
            continue;
...
# 4605 "0_Src/0_AppSw/Fat_fs/ff.c" 2
    ...
    )
{
    ...
    if (c == '\n')
        f_putc ('\r', fil);
# 4644 "0_Src/0_AppSw/Fat_fs/ff.c"
    s[0] = (BYTE) c;
    btw = 1;
    ...
    )
{
    ...
    for (cc = res = 0; cc != (-1); res += cc)
    {
        ...
        case 'S':
            ...
            continue;
        case 'C':
            cc = f_putc ((TCHAR) __builtin_va_arg(arp,int), fil);
            continue;
        case 'B':
            r = 2;
            break;
        ...
        v = (f & 4) ? (ULONG) __builtin_va_arg(arp,long)
                                            : ((d == 'D') ? (ULONG) (long) __builtin_va_arg(arp,int) : (ULONG) __builtin_va_arg(arp,unsigned int)
