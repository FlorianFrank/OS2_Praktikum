# 1 "0_Src/OS/cpu/tricore/src/ee_tc_context.c"
# 1 "C:\\betriebssys\\BaseFramework_TC26xB//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "0_Src/OS/cpu/tricore/src/ee_tc_context.c"
# 51 "0_Src/OS/cpu/tricore/src/ee_tc_context.c"
...
# 52 "0_Src/OS/cpu/tricore/src/ee_tc_context.c" 2




static inline void __attribute__((always_inline)) EE_tc_task_save( EE_UTID utid )
{
  EE_tc_tasks_RA[utid] = EE_tc_get_RA();
}


static inline void __attribute__((always_inline)) EE_tc_task_restore( EE_UTID utid )
{
  EE_ADDR const ra = EE_tc_tasks_RA[utid];
  EE_tc_set_RA(ra);
}

static inline void __attribute__((always_inline)) EE_tc_stack_save( EE_UREG tos )
{
  struct EE_TC_TOS * const p_tos = &EE_tc_system_tos[tos];

  p_tos->ram_tos = EE_tc_get_SP();
  p_tos->pcxi_tos = (__extension__({ unsigned __res; __asm__ volatile ("mfcr %0, LO:" "0xFE00" : "=d" (__res) :: "memory"); __res; }));
}
...
# 116 "0_Src/OS/cpu/tricore/src/ee_tc_context.c"
